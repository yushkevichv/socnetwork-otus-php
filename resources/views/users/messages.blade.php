...
<div class="bg-white shadow overflow-hidden sm:rounded-lg">
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 w-2/3" >
                        <div class="flex justify-between ">
                            <div class="text-sm leading-5 font-medium text-gray-900">{{ $message->author->name }} {{ $message->author->last_name }}</div>
                            <div class="text-sm right-auto leading-5 font-medium text-gray-900">{{ $message->created_at }} </div>
                        </div>
                        <div class="text-sm pt-4 leading-5 font-medium text-gray-900">{{ $message->text }} </div>
                    </td>
                </tr>
            @endforeach
    ...
    <div class="bg-white px-4 py-3 flex items-right justify-between border-t border-gray-200 ">
        ...
        <form class="flex-auto" action="{{ route('messages.store', ['id' => $chat->id]) }}" method="POST">
            @csrf
